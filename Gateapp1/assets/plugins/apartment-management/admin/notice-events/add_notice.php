		    ...
		    <div class="panel-body"><!--PANEL BODY-->
				...
				<form name="notice_form" action="" method="post" class="form-horizontal" id="notice_form" enctype="multipart/form-data">
					...
					<div class="form-group"><!--TITLE-->
						...
						<div class="col-sm-8">
							<input id="notice_title" maxlength="50" class="form-control text-input validate[required,custom[onlyLetter_specialcharacter]]" type="text"  value="<?php if($edit){ echo $result->notice_title;}elseif(isset($_POST['notice_title'])) echo esc_attr($_POST['notice_title']);?>" name="notice_title">
						</div>
					</div>
					...
					<div class="form-group"><!---NOTICETYPE--->
						...
						<?php $noticetype = "Announcement"; if($edit){ $noticetype=$result->notice_type; }elseif(isset($_POST['notice_type'])) {$noticetype=$_POST['notice_type'];}?>
							<label class="radio-inline">
							 <input type="radio" value="Announcement" class="tog validate[required]" name="notice_type"  <?php  checked( 'Announcement', $noticetype);  ?>/><?php esc_html_e('Announcement','apartment_mgt');?>
							</label>
							<label class="radio-inline">
							  <input type="radio" value="Proposal" class="tog validate[required]" name="notice_type"  <?php  checked( 'Proposal', $noticetype);  ?>/><?php esc_html_e('Proposal','apartment_mgt');?> 
							</label>
