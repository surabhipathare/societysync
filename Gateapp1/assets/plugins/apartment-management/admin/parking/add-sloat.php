		...
		<div class="panel-body"><!---PANEL BODY--->		
				...
				<form name="sloat_form" action="" method="post" class="form-horizontal" id="sloat_form">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input id="sloat_name" maxlength="20" class="form-control validate[required,custom[onlyLetter_specialcharacter]] text-input" type="text"  value="<?php if($edit){ echo esc_attr($result->sloat_name);}elseif(isset($_POST['sloat_name'])) echo esc_attr($_POST['sloat_name']);?>" name="sloat_name">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<?php $sloattype = "member"; if($edit){ $sloattype=$result->sloat_type; }elseif(isset($_POST['sloat_type'])) {$sloattype=$_POST['sloat_type'];}?>
							...
							<label class="radio-inline">
							  <input type="radio" value="guest" class="tog validate[required]" name="sloat_type"  <?php  checked( 'guest', $sloattype);  ?>/><?php esc_html_e('Guest','apartment_mgt');?> 
							</label>
						</div>
					</div>
